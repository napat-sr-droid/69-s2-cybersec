...
module.exports = {
  ...
  async forgotPassword(ctx) {
    ...
    // NOTE: Best-effort send — mirrors the built-in admin forgot-password behaviour.
    // The prawee/strapi image has no real SMTP provider (email falls back to the
    // 'sendmail' package, which Google's MX rejects with 550), so a real email
    // never arrives here. Log the error instead of failing the request, and keep
    // the reset code stored so /api/auth/reset-password still works end-to-end.
    try {
